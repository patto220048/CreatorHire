...
import { LayoutDashboard, PlusCircle, Briefcase, LogOut } from "lucide-react";
...
export default function CreatorSidebar({ user }: SidebarProps) {
  ...
  const menuItems = [
    { name: "Tổng quan", path: "/creator", icon: LayoutDashboard },
    { name: "Đăng tin mới", path: "/creator/jobs/new", icon: PlusCircle },
    { name: "Dự án của tôi", path: "/creator/jobs", icon: Briefcase },
  ];
  ...
  const handleLogout = async () => {
    // Gọi Client Supabase để Sign Out
    try {
  ...
  return (
    <div
      ...
      style={{ opacity: 0 }} // Tránh giật layout trước khi GSAP chạy
    >
      {/* Brand Header */}
      ...
      <div className="p-6 border-b border-stone/10 bg-stone/5 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-brand-green/20 text-brand-green border border-brand-green/30 flex items-center justify-center font-bold text-sm">
          {user.fullName ? user.fullName.substring(0, 2).toUpperCase() : "CR"}
        </div>
        <div className="overflow-hidden">
          <h4 className="text-xs font-semibold text-on-dark truncate">{user.fullName || "Creator"}</h4>
          <p className="text-[10px] text-on-dark-muted truncate">{user.email || "creator@creatorhire.vn"}</p>
        </div>
      </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Hiệu ứng line hover khi không active */}
              {!isActive && (
